CREATE OR REPLACE PACKAGE BODY etladmin.pkg_etl_utils AS
/*
  =================================================================================================
  ...
  30-Jul-2020, OK: in the 2nd FIND_TABLE, commented-out the call to THE 1st FIND_TABLE.
  29-Jul-2020, OK: added explanation that Delete condition should start with "IF ".
  15-Jul-2020, OK: bug fix - removed RAPLACE(v_src. 's.') from the action "Generating DML command". 
  04-May-2020, OK: in P_WHR of DELETE_DATA, "WHERE" is now assumed.
  26-Apr-2020, OK: adjusted log levels.
...
*/
  ...
  BEGIN
    xl.begin_action('Looking for table/view', p_name, 10, $$PLSQL_UNIT);
    ...
    --find_table(p_schema, p_table); -- OK: commented-out to improve performance
  ...
  (
    ...
    BEGIN
      ...
      xl.begin_action('Collecting and analyzing metadata', 'Started', 10, $$PLSQL_UNIT);

      xl.begin_action('Collecting metadata', 'Started', 10, $$PLSQL_UNIT);
      ...
      xl.begin_action('Setting V_INS_COLS and V_SEL_COLS', 'Started', 10, $$PLSQL_UNIT);
      SELECT
      ...
      IF v_operation IN ('MERGE','UPDATE') THEN
        xl.begin_action('Setting V_MATCH_COLS', 'Started', 10, $$PLSQL_UNIT);
        ...
        xl.begin_action('Setting V_UPD_COLS', 'Started', 10, $$PLSQL_UNIT);
        ...
        xl.begin_action('Setting V_UPD_LIST', 'Started', 10, $$PLSQL_UNIT);
        ...
        WHEN v_match_cols = 'ROWID' THEN
          ...
          IF b_del_notfound THEN
            xl.begin_action('Checking that the source has the column ETL$SRC_INDICATOR','Started', 10, $$PLSQL_UNIT);
        ...
        ELSE -- matching on regular columns, not on ROWID
          xl.begin_action('Setting V_ON_LIST', 'Started', 10, $$PLSQL_UNIT);
          SELECT concat_v2_set(CURSOR(
            SELECT
              CASE t.nullable
          ...
          xl.begin_action('Setting V_CHANGED_COND', 'Started', 10, $$PLSQL_UNIT);
      ...
      xl.begin_action('Checking that all referenced columns actually exist', 'Started', 10, $$PLSQL_UNIT);
      FOR r IN
      (
    ...
    BEGIN
      IF v_view_name IS NOT NULL THEN
        ...
        xl.begin_action('Dropping view '||v_view_name, v_cmd, 10, $$PLSQL_UNIT);
      ...
      IF v_tab_type_name IS NOT NULL THEN
        ...
        xl.begin_action('Dropping type '||v_tab_type_name, v_cmd, 10, $$PLSQL_UNIT);
        EXECUTE IMMEDIATE v_cmd;
        xl.end_action;
      END IF;
      ...
      IF v_obj_type_name IS NOT NULL THEN
        ...
        xl.begin_action('Dropping type '||v_obj_type_name, v_cmd, 10, $$PLSQL_UNIT);
        EXECUTE IMMEDIATE v_cmd;
        xl.end_action;
      END IF;
    END;
  ...
  BEGIN
    ...
    IF p_delete IS NOT NULL THEN
      ...
      xl.begin_action('Parsing P_DELETE', 'Started', 10, $$PLSQL_UNIT);
    ...
    IF p_versions IS NOT NULL THEN
      xl.begin_action('Parsing P_VERSIONS', 'Started', 10, $$PLSQL_UNIT);
    ...
    IF p_generate IS NOT NULL THEN
      xl.begin_action('Parsing P_GENERATE', 'Started', 10, $$PLSQL_UNIT);
    ...
    IF UPPER(p_source) LIKE '%SELECT%' THEN
      ...
      xl.begin_action('Creating view '||v_view_name, v_cmd, 10, $$PLSQL_UNIT);
      EXECUTE IMMEDIATE v_cmd;
      xl.end_action;
    ...
    ELSE
      ...
      EXCEPTION
       WHEN remote_object THEN
        ...
        xl.begin_action('Creating a local view '||v_view_name||' for the remote source "'||p_source||'"', v_cmd, 10, $$PLSQL_UNIT);
        EXECUTE IMMEDIATE v_cmd;
        xl.end_action;
    ...
    IF v_operation = 'REPLACE' THEN
      xl.begin_action('Truncating table '||p_target, 'Started', 10, $$PLSQL_UNIT);
      EXECUTE IMMEDIATE 'TRUNCATE TABLE '||p_target;
      v_operation := 'INSERT';
      xl.end_action;
    END IF;
    
    xl.begin_action('Generating source expression', 'Started', 10, $$PLSQL_UNIT);
    v_src := 
...
WHERE '|| p_where
    ...
    xl.begin_action('Generating DML command', 'Started', 10, $$PLSQL_UNIT);
    v_cmd :=
...
LOG ERRORS INTO '||v_err_schema||'.'||v_err_table||' (:tag) REJECT LIMIT UNLIMITED'
    ...
    IF p_commit_at > 0 THEN -- incremental load with commit afrer each portion
      ...
      xl.begin_action('Creating object type '||v_obj_type_name, v_act, 10, $$PLSQL_UNIT);
      ...
      xl.begin_action('Creating table type '||v_tab_type_name, v_act, 10, $$PLSQL_UNIT);
      EXECUTE IMMEDIATE v_act;
      xl.end_action;

      xl.begin_action('Converting DML command into PL/SQL block with LOOP', 'Started', 10, $$PLSQL_UNIT);
...
'     END || v_cmd || ';
    ...
    xl.begin_action(:act, ''Continue ...'', 5, ''PKG_ETL_UTILS'');
  END LOOP;
