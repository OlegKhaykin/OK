...
with
  ...
  (
    ...
    JOIN dba_objects                  o
     ...
     --and o.object_name like 'MEMBERDERIVEDFACT_UR'
    JOIN gv$session                   s
      ON s.sid = lo.session_id AND s.inst_id = lo.inst_id
  )
select * from det;
select distinct locked_object from det
--where locked_object like 'CSID%'
;  
...
select o.object_name, s.*, lck.* 
from dba_objects                        o
join gv$lock                            lck 
  on lck.id1 = o.object_id 
join gv$session                         s
  on s.inst_id = lck.inst_id and s.sid = lck.sid 
--where o.owner = 'CSID' and o.object_name like 'TMP%'
;
