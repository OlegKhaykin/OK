CREATE OR REPLACE VIEW vw_psa_uat_sb1_pprd_comsetup_dbg AS
WITH
  -- 02-Mar-2021,R.Donakonda: created UAT test dbg view for PurchasedProductCommsetup table
  new_results AS
  (
    SELECT --+ materialize
      ppsl.psuid,ff.* 
...
SELECT 'PurchasedProductCommsetup : NEW'  AS Compare, n.* FROM (SELECT * FROM new_results) n
UNION ALL
SELECT 'PurchasedProductCommsetup : OLD'  AS Compare, o.* FROM (SELECT * FROM old_results) o;

GRANT SELECT ON vw_psa_uat_sb1_pprd_comsetup_dbg TO deployer, ods_dml;
