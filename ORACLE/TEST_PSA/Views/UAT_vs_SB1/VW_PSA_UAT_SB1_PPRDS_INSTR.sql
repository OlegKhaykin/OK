CREATE OR REPLACE VIEW vw_psa_uat_sb1_pprds_instr AS
WITH
  -- 04-Jan-2021,UAT Test view R.Donakonda: created
  new_results AS
  (
    SELECT --+ materialize
      ppsl.psuid,pp.productcd,ppi.existingaccountpackageflg,
      ppi.suggestedaccountpackageid,ppi.exceptioncctext,ppi.ccinclusionruleid
  ...
  (
    ...
    JOIN ahmadmin.PurchasedProductSettings@sb1                            pps
      ON pps.PurchasedProductID = pp.PurchasedProductID
    JOIN ahmadmin.PurchasedProductInstructions@sb1                        ppi
      ON ppi.PurchasedProductSettingsSKEY = pps.PurchasedProductSettingsSKEY   
  )
SELECT
  'PurchasedProductInstructions : NEW minus OLD'  AS Compare,
  n.* 
FROM (SELECT * FROM new_results MINUS SELECT * FROM old_results) n
UNION ALL
SELECT
  'PurchasedProductInstructions : OLD minus NEW'  AS Compare,
  o.* 
FROM (SELECT * FROM old_results MINUS SELECT * FROM new_results) o;

GRANT SELECT ON vw_psa_uat_sb1_pprds_instr TO deployer, ods_dml;
