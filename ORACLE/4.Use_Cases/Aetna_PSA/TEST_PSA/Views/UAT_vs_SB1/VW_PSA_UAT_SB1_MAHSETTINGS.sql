CREATE OR REPLACE VIEW vw_psa_uat_sb1_mahsettings AS
WITH
  -- 04-Jan-2021,UAT-Test View R.Donakonda: created
  new_results AS
  (
    SELECT --+ materialize
     ppsl.psuid,pp.ProductCD,ms.CustomURL,ms.last4digitsofssnreqflg,
     ms.additionalregistrationfieldflg,ms.suppressemailnotificationflg
  ...
  (
    ...
    JOIN ahmadmin.PurchasedProductSettings@sb1                            pps
      ON pps.PurchasedProductID = pp.PurchasedProductID
    JOIN ahmadmin.MAHsettings@sb1                                         ms
      ON ms.PurchasedProductSettingsSKEY = pps.PurchasedProductSettingsSKEY 
  )
SELECT
  'MAHsettings : NEW minus OLD'  AS Compare,
  n.* 
FROM (SELECT * FROM new_results MINUS SELECT * FROM old_results) n
UNION ALL
SELECT
  'MAHsettings : OLD minus NEW'  AS Compare,
  o.* 
FROM (SELECT * FROM old_results MINUS SELECT * FROM new_results) o;

GRANT SELECT ON vw_psa_uat_sb1_mahsettings TO deployer, ods_dml;
