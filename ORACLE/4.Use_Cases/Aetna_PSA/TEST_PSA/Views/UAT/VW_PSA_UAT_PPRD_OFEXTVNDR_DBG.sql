CREATE OR REPLACE VIEW vw_psa_uat_pprd_ofextvndr_dbg AS
WITH
  ...
  (
    ...
    JOIN ahmadmin.PurchasedProductOFFERING@psa                            ppo
      ON ppo.PurchasedProductID = pp.PurchasedProductID
    JOIN ahmadmin.PurchasedProdoffngEXTVENDOR@psa                         ppe
      ON ppe.PurchasedProductOfferingSKEY = ppo.PurchasedProductOfferingSKEY 
  )
SELECT 'PurchasedProdoffngEXTVENDOR : NEW' AS Compare,  n.* FROM (SELECT * FROM new_results) n
UNION ALL
SELECT 'PurchasedProdoffngEXTVENDOR : OLD' AS Compare,  o.* FROM (SELECT * FROM old_results) o;

GRANT SELECT ON vw_psa_uat_pprd_ofextvndr_dbg TO deployer, ods_dml;
